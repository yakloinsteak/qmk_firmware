...
static void render_logo(void) {
    oled_set_cursor(0, 0);  // pin to top-left; the OLED cursor persists across
                            // frames, so without this the logo drifts by
                            // whatever the previous layer's status text left.
